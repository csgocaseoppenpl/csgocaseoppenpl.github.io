...
var items = [
  ...
  {name:'Ultimate Case', price:1250.00, class:'standard', case:'ultimate', type:'case', needkey:false, rankNeeded:1},
	...
	{name:'Bayonet | Marble Fade', price:418.77, class:'exceedingly_rare', case:'knife csgo_weapon csgo_weapon2 csgo_weapon3 vanguard ultimate', type:'skin', stattrak:true},
	...
	{name:'Falchion | Marble Fade', price:384.00, class:'exceedingly_rare', case:'knife spectrum falchion ultimate', type:'skin', stattrak:true},
	...
	{name:'Shadow Daggers | Marble Fade', price:314.13, class:'exceedingly_rare', case:'knife shadow ultimate', type:'skin', stattrak:true},
	...
	{name:'Butterfly | Marble Fade', price:788.55, class:'exceedingly_rare', case:'knife spectrum ultimate', type:'skin', stattrak:true},
	...
	{name:'Karambit | Marble Fade', price:622.31, class:'exceedingly_rare', case:'knife csgo_weapon csgo_weapon2 csgo_weapon3 vanguard ultimate', type:'skin', stattrak:true},
	...
	//  Spectrum case
	{name:'AK-47 | Bloodsport', price:56.95, class:'covert', case:'spectrum stattrak4 ultimate', type:'skin', stattrak:true},
	...
	{name:'AWP | Dragon Lore', price:729.53, class:'covert', case:'cobblestone ultimate', type:'skin', stattrak:false},
	...
	{name:'AWP | Medusa', price:602.95, class:'covert', case:'gods_and_monsters ultimate', type:'skin', stattrak:false},
	...
	{name:'MAC-10 | Neon Rider', price:1.77, class:'covert', case:'chroma2 stattrak4 ultimate', type:'skin', stattrak:true},
	...
	{name:'Desert Eagle | Sunset Storm å¼', price:4.90, class:'restricted', case:'risingsun', type:'skin', stattrak:false},
	{name:'Desert Eagle | Sunset Storm å£±', price:4.99, class:'restricted', case:'risingsun', type:'skin', stattrak:false},
...
var upgrades = [
	{name:'Bank',price:42.32,dec:'Earn 500$ more when offline',priceboost:4.9},
...
for (var i = 0; i < upgrades.length; i++) {
	...
	if (userdata.upgrades[upgrades[i].name].amount > 1000) {
		userdata.upgrades[upgrades[i].name].amount = 1000;
	}
}
	...
	if (checkKey == true || checkKey == 2) {
		...
		for (var i = 0; i < 40; i++) {
			...
			if (item != undefined) {
				...
				a.innerHTML = '<div class="image" style="background-image:url(https://mtsl.dk/csgo/images/' + item.name.replace(' | ', '').replace('å£±', '1').replace('å¼', '2').replace(/ /g, '').replace(/:/g, '').replace(/'/g, '') + '.png)"></div><div class="title">' + item.name + '</div>';
		...
		for (var i = 0 + p*ipp; i < ipp + p*ipp; i++) {
			if (userdata.inv[i] != undefined) {
				...
				if (data != false) {
					...
					iteminner.innerHTML = '<div class="image" style="background-image:url(https://mtsl.dk/csgo/images/' + data.name.replace(' | ', '').replace('å£±', '1').replace('å¼', '2').replace(/ /g, '').replace(/:/g, '').replace(/'/g, '') + '.png)"></div><div class="title">' + data.name + '</div>';
					...
					iteminner.addEventListener('click', function() {
						...
						$('#inspect_image').style.backgroundImage = 'url(https://mtsl.dk/csgo/images/' + data.name.replace(' | ', '').replace('å£±', '1').replace('å¼', '2').replace(/ /g, '').replace(/:/g, '').replace(/'/g, '') + '.png)';
...
function rendUpgrades() {
	...
	for (var i = 0; i < upgrades.length; i++) {var g = document.createElement('div');
		...
		g.innerHTML = '<span class="name">' + upgrades[i].name + '</span>' + '<span class="dec">' + upgrades[i].dec + '. (' + (userdata.upgrades[upgrades[i].name].amount < 1000 ? userdata.upgrades[upgrades[i].name].amount : 'MAX') + ')</span>';
		...
		btn.setAttribute('below1000', userdata.upgrades[upgrades[i].name].amount);

		btn.addEventListener('click', function() {
			var i = this.getAttribute('i');
			var price = (upgrades[i].price * (1 + userdata.upgrades[upgrades[i].name].amount * upgrades[i].priceboost));
			if (price <= userdata.money && userdata.upgrades[upgrades[i].name].amount < 1000) {
...
function update() {
	if (canUpdate) {
		...
		for (var i = 0; i < disabledIf.length; i++) {
			...
			if (disabledIf[i].getAttribute('below1000') != undefined) {
				if (disabledIf[i].getAttribute('below1000') > 999) {
					disabledIf[i].disabled = true;
				}
			}
			if (disabledIf[i].getAttribute('min') != undefined) {
	...
	for (var i = 0; i < userdata.inv.length; i++) {
		if (userdata.inv[i] != undefined) {
			...
			if (data.type == 'skin') {
				...
				itemD.innerHTML = '<div class="image" style="background-image:url(https://mtsl.dk/csgo/images/' + data.name.replace(' | ', '').replace('å£±', '1').replace('å¼', '2').replace(/ /g, '').replace(/:/g, '').replace(/'/g, '') + '.png)"></div><div class="title">' + data.name + '</div><div class="price">' +
...
function showItems(items) {
	...
	for (var i = 0; i < items.length; i++) {
		...
		item.innerHTML = '<div class="image" style="background-image:url(https://mtsl.dk/csgo/images/' + items[i].name.replace(' | ', '').replace('å£±', '1').replace('å¼', '2').replace(/ /g, '').replace(/:/g, '').replace(/'/g, '') + '.png)"></div><div class="title">' + items[i].name + '</div><div class="price">' +
...
if (lte >= userdata.upgrades['Bank'].amount * 50 + 500) {
	lte = userdata.upgrades['Bank'].amount * 50 + 500;
}
...
function incase() {
	...
	for (var i = 0; i < a.length; i++) {
		...
		it.innerHTML = '<div class="image" style="background-image:url(https://mtsl.dk/csgo/images/' + item.name.replace(' | ', '').replace('å£±', '1').replace('å¼', '2').replace(/ /g, '').replace(/:/g, '').replace(/'/g, '') + '.png)"></div><div class="title">' + item.name + '</div>';
...
for (var i = 0; i < items.length; i++) {
	if (items[i].type == 'skin') {
		...
		for (var h = 0; h < 5; h++) {
			var itemD = document.createElement('div');
			var w = wl[h];
			itemD.innerHTML = '<div class="image" style="background-image:url(https://mtsl.dk/csgo/images/' + items[i].name.replace(' | ', '').replace('å£±', '1').replace('å¼', '2').replace(/ /g, '').replace(/:/g, '').replace(/'/g, '') + '.png)"></div><div class="title">' + items[i].name + '</div><div class="price">' + Math.ceil(items[i].price*wtp[w]) + ' coins</div>';
			itemD.setAttribute('i', i);
			...
			if (items[i].stattrak == true) {
				...
				itemD.innerHTML = '<div class="image" style="background-image:url(https://mtsl.dk/csgo/images/' + items[i].name.replace(' | ', '').replace('å£±', '1').replace('å¼', '2').replace(/ /g, '').replace(/:/g, '').replace(/'/g, '') + '.png)"></div><div class="title">' + items[i].name + '</div><div class="price">' + Math.ceil(items[i].price*3*wtp[w]) + ' coins</div>';
				itemD.setAttribute('i', i);
				itemD.setAttribute('w', w);
